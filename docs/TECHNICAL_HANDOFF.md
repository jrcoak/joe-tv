...
- Playback follows the conventional Siri Remote contract: Select/Up/Down reveals controls, Down again opens Quick Switch, Play/Pause toggles immediately, long Select returns to the last live stream, seekable Left/Right input skips or scrubs, and Page Up/Down changes live channels. Optional blind Up/Down channel surfing is off by default and persisted in Settings.
...
- **Player switching is explicit.** A short Select press reveals Play/Pause, Favorite, and Guide controls; a 0.6-second Select hold opens one continuous Recent-then-Favorites rail, and Down from the controls remains a second path to it. Recent history is session-only, capped at four, newest first, URL-free, and deduplicated against favorites. The current stream is omitted. With player chrome hidden, Up/Down wraps through only the enabled Live TV lineup in configured order; it does nothing during Sports or ESPN+ playback. Selecting a rail card or surfing to a channel resolves a fresh stream identity, starts live content at the live edge, and restores the previous session if preparation fails. Back reverses the overlay layers before leaving playback.
