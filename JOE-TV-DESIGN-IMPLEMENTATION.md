...
- **Home:** The hero follows the currently focused favorite channel and defaults to the first favorite on entry. The Favorites rail is built from the user's persisted, enabled favorite channels and their real EPG data; sports promotion remains on the Sports screen.
...
- **My Teams and broadcast memory:** Needs user preference models and migration behavior. Channel favorites now persist separately from channel enable/disable state.
